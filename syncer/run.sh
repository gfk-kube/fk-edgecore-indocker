...
# registry.deploy.xxx.com.ssl:8143: #DST2_DOMAIN
sed -i "s/.*#DST2_DOMAIN/${DOCKER_REGISTRY_DST2_DOMAIN}:8143: #DST2_DOMAIN/g" $authyml
...
# /etc/docker/certs.d/deploy.xxx.com.ssl:8143/deploy.xxx.com.ssl.crt

# --proc 1 #多了hub取不到
./image-syncer --proc 1 --auth $authyml --images ./images.yml --arch=amd64 --arch=arm64 #--arch amd64,arm64
}
...
cat $1 |grep -Ev "^#|^$" |awk '{print $1}' |while read one; do
   ...
   src=$one
   plain=$(echo $one |sed "s^/^-^g")
   dst="registry.cn-shenzhen.aliyuncs.com/infrasync/$plain"
   # echo "$src: $dst" >> images.yml

   # dst2="172.25.21.60:8143/infrasync/$dst"
   dst2="$DOCKER_REGISTRY_DST2_DOMAIN:8143/infrasync222/$plain"
