...
case "$1" in
builder)
    # cd multiplat-builder/
      bash imgs/multiplat-builder/1.binData_outhere.sh
      bash imgs/multiplat-builder/2.loadBins.sh
      img=multiplat-builder:v1.1-up2
      docker build -t $repo/$ns/$img -f ./imgs/multiplat-builder/Dockerfile .
      echo docker push $repo/$ns/$img
